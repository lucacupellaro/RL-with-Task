...
for t=2:experiment_length
   ...
   [action, isExploration(t)] = scegliAzioneEpsGreedy(actionValues(:,t-1), epsilon);
    ...
    step_size = 1/actionNumber(action, t);
...
% Grafico 1: esplorazione puntuale
nexttile
plot(isExploration)
title('Exploration (1) vs Exploitation (0)')
xlabel('Time step t')
ylabel('Exploration flag')
ylim([-0.1 1.1])
grid on

% Grafico 2: tasso di esplorazione medio
nexttile
window = 200;
explorationRate = movmean(isExploration, window);
plot(explorationRate, 'LineWidth', 1.2)
title('Exploration Rate (Moving Average)')
xlabel('Time step t')
